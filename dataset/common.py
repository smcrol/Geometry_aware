...
def get_overlap_mask(ref_points, src_points, transform, matching_radius):
    r"""Compute sop region mask

    Returns:
        ref_overlap: Whether each reference point is in the sop region
        src_overlap: Whether each source point is in the sop region
    """
    src_points = np_transform(transform, src_points)

    # Mask for reference's sop region
