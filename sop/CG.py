...
class CGModule(nn.Module):
    ...
    def forward(self, src_feature, tgt_feature):
        '''
        Context-Guided Model for initial alignment and sop score.
        :param src: (B, N, 3)
        :param tgt: (B, M, 3)
        :return: T0: (B, 3, 4), OX: (B, N, 2), OY: (B, M, 2)
        '''

        # sop prediction and overlapping mask prediction
        x_ol = self.decoder_ol(src_feature, pooling=False)
        y_ol = self.decoder_ol(tgt_feature, pooling=False)

        x_ol_mask_u = torch.argmax(x_ol, dim=1).detach()
        y_ol_mask_u = torch.argmax(y_ol, dim=1).detach()

        x_ol_mask = x_ol_mask_u.unsqueeze(1)
        y_ol_mask = y_ol_mask_u.unsqueeze(1)
        ...
        return (x_ol, y_ol,
                src_mask_feature, tgt_mask_feature,
                x_ol_mask_u, y_ol_mask_u)
