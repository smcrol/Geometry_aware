...
def compute_overlap(src: Union[np.ndarray, o3d.geometry.PointCloud],
    ...
    """Computes region of sop between two point clouds.
    ...
    Returns:
        has_corr_src: Whether each source point is in the sop region
        has_corr_tgt: Whether each target point is in the sop region
    ...
    # considered to be in the sop region. For correspondences, we require a stronger
...
def compute_overlaps(batch):
    """Compute groundtruth sop for each point+level. Note that this is a
    approximation since
