...
from sop.overlap_utils.preprocessGPU import compute_overlaps
# modelnet part
    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)):
                ...
                p = len(inputs['stack_lengths']) - 1
                len_node = inputs['stack_lengths'][-1][0]
                inputs['overlap_pyr'] = compute_overlaps(inputs)
                node_overlap = torch.reshape(inputs['overlap_pyr'][f'pyr_{p}'], (1, -1))
                gt_node_corr = inputs['node_corr']

                rot = inputs['rot']
                trans = inputs['trans']

                src_candidates_c, tgt_candidates_c, local_scores, node_corr, node_corr_conf, src_pcd_sel, tgt_pcd_sel, x_ol, y_ol, \
                    src_patch, tgt_patch, src_val_mask, tgt_val_mask = self.model.forward(inputs)
                ...
                data['matching_scores'] = local_scores.cpu()
                data['node_corr_scores'] = node_corr_conf.cpu()

                data['node_lable'] = node_overlap.cpu()
                data['node_length'] = len_node.cpu()
                data['node_corr'] = node_corr.cpu()
                data['gt_node_corr'] = gt_node_corr
                data['pre_x_ol'] = x_ol.cpu()
                data['pre_y_ol'] = y_ol.cpu()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for _ in tqdm(range(num_iter)):  # loop through this epoch
                ...
                src_candidates_c, tgt_candidates_c, local_scores, node_corr, node_corr_conf, src_pcd_sel, tgt_pcd_sel, x_ol, y_ol, \
                src_patch, tgt_patch = self.model.forward(inputs)
                correspondences, corr_conf = get_fine_grained_correspondences(local_scores, mutual=False, supp=False, node_corr_conf=node_corr_conf)
                ########################################
                ...
                data['src_pcd'], data['tgt_pcd'] = src_pcd.cpu(), tgt_pcd.cpu()
                data['src_node'], data['tgt_node'] = src_node.cpu(), tgt_node.cpu()
                data['src_candidate'], data['tgt_candidate'] = src_candidates_c.view(-1,
                                                                                     3).cpu(), tgt_candidates_c.view(-1,
                                                                                                                     3).cpu()
                data['src_candidate_id'], data['tgt_candidate_id'] = src_pcd_sel.cpu(), tgt_pcd_sel.cpu()
                data['rot'] = rot.cpu()
                data['trans'] = trans.cpu()
                ...
                data['node_corr'] = node_corr.cpu()
                data['gt_node_corr'] = gt_node_corr
                data['src_patch'] = src_patch.cpu()
                data['tgt_patch'] = tgt_patch.cpu()
                data['pre_x_ol'] = x_ol.cpu()
                data['pre_y_ol'] = y_ol.cpu()
                data['pre_tsfm'] = ts_est

                torch.save(data, f'{self.snapshot_dir}/KITTI/{count}.pth')
                count += 1
