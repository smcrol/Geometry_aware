...
from lib.losses import ModelLoss
...
class Trainer(object):
    ...
    def _load_pretrain(self, resume):
        ...
        if os.path.isfile(resume):
            ...
            self.start_epoch = state['epoch']  # state['epoch']
            ...
            # # # 冻结模型参数
            # # # modules_to_freeze = ['encoder', 'decoder']
            # for name, param in self.model.named_parameters():
            #     # 检查模块名是否在要冻结的列表中，但排除encoder.backbone
            #     # module_name = name.split('.')[0] if '.' in name else ""
            #     if 'encoder' in name and 'backbone' not in name and 'position' not in name :  # and 'backbone' not in name and 'position' not in name
            #         param.requires_grad = False
            #         print(f"Parameter '{name}' is frozen. requires_grad: {param.requires_grad}")
            #     else:
            #         param.requires_grad = True
            #         print(f"Parameter '{name}' is not frozen. requires_grad: {param.requires_grad}")
    ...
    def inference_one_batch(self, input_dict, phase):
        ...
        if (phase == 'train'):
            ...
            # forward pass
            ###############
            scores, pre_x_ol, pre_y_ol, local_scores, local_scores_gt = self.model.forward(input_dict)
            matching_mask = input_dict['matching_mask']

            ###############
            # get loss
            ###############
            stats = self.compute_loss(input_dict, scores[0], matching_mask, pre_x_ol, pre_y_ol, local_scores, local_scores_gt)
            c_loss = stats['matching_loss'] + stats['overlap_loss'] + stats['local_matching_loss']
            c_loss.backward()
        else:
            self.model.eval()
            with torch.no_grad():
