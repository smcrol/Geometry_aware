...
def read_trajectory(filename, dim=4):
    ...
    Returns:
    final_keys (dict): indices of pairs with more than 30% sop (only this ones are included in the gt file)
    ...
    with open(filename) as f:
        ...
        traj = np.asarray(traj, dtype=np.float64).reshape(-1,dim,dim)
...
def read_trajectory_info(filename, dim=6):
    ...
    cov_matrix = np.asarray(info_list, dtype=np.float64).reshape(-1,dim,dim)
...
def extract_corresponding_trajectors(est_pairs,gt_pairs, gt_traj):
    ...
    Args:
    est_pairs (numpy array): indices of point cloud pairs with enough estimated sop [m, 3]
...
def write_trajectory(traj,metadata, filename, dim=4):
    ...
    with open(filename, 'w') as f:
        for idx in range(traj.shape[0]):
            # Only save the transfromation parameters for which the sop threshold was satisfied
            if metadata[idx][2]:
                p = traj[idx,:,:].tolist()
