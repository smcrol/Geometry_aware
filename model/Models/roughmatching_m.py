...
import numpy as np
from sop.CG import CGModule
from sop.lisms.corrdinate_mask import canonicalize_point_cloud
from sop.lisms.corrdinate_encode import PointFeatureEmbedDeep
...
class RoughMatchingModel(nn.Module):
    def __init__(self, config):
        ...
        # CG Model part
        self.CG = CGModule(False)
        self.point_encode = PointFeatureEmbedDeep()
        ##############################################
    ...
    def forward(self, batch):
        ...
        x_ol, y_ol, src_mask_feature, tgt_mask_feature, x_ol_mask_u, y_ol_mask_u = self.CG(src_feats_c, tgt_feats_c)
        centered_src_c = canonicalize_point_cloud(src_node_c, x_ol_mask_u)
        centered_tgt_c = canonicalize_point_cloud(tgt_node_c, y_ol_mask_u)
        # src_mask_feature_en = src_mask_feature + self.point_encode(centered_src_c)
        # tgt_mask_feature_en = src_mask_feature + self.point_encode(centered_tgt_c)
        src_mask_feature_en = torch.cat(
            [src_mask_feature, self.point_encode(centered_src_c)],
            dim=1
        )

        tgt_mask_feature_en = torch.cat(
            [tgt_mask_feature, self.point_encode(centered_tgt_c)],
            dim=1
        )
        dim = src_mask_feature_en.size(1)  # (256,)
        scores = torch.einsum('bdn,bdm->bnm', src_mask_feature_en, tgt_mask_feature_en)  # (1,313,216)
        scores = scores / dim ** 0.5  # 归一化得分

        scores = log_optimal_transport(scores, None, None, self.bin_score, iters=self.sinkhorn_iters)  # (1,314,217)

        #########################################
        # 4. Decoder part
        decoder_input_feats = torch.cat([src_feats_c, tgt_feats_c], dim=-1)
        final_feats = self.decoder(batch, decoder_input_feats, skip_x)  # (1,31168,32)
        src_final_f, tgt_final_f = final_feats[:len_src_c, :], final_feats[len_src_c:, :]  # (1,17692,32);(1,13539,32)
        ...
        row_score_mask = src_node_mask.unsqueeze(2).expand(src_node_mask.shape[0], src_node_mask.shape[1], src_node_mask.shape[1])  # (128,64,64)
        col_score_mask = tgt_node_mask.unsqueeze(1).expand(tgt_node_mask.shape[0], tgt_node_mask.shape[1], tgt_node_mask.shape[1])  # (128,64,64)
        ...
        if not self.test:
            ...
            return (src_pcd_sel_c.view(-1, 3), tgt_pcd_sel_c.view(-1, 3), local_scores, node_corr,
                    node_corr_conf, src_pcd_sel, tgt_pcd_sel, x_ol, y_ol, src_patch, tgt_patch, src_val_mask, tgt_val_mask)
